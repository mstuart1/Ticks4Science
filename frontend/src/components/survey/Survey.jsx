...
const Survey = () => {
  ...
  const handleNumTicks = ({ target: { value } }) => {
    //!! on some mobile, the up down arrows disappear from the numTicks input, and if someone deletes the previous number to set a new number, it sets the numTicks to zero and sets ticks to an empty array
    setNumTicks(value)
    if (value < ticks.length) {
      ...
      if (ticks.length === 0) {
        for (let step = 0; step < value; step++) {
           freshTicks.push({ ...initialTick, key: step })
        }
        
      } else {
         freshTicks = [...ticks, { ...initialTick, key: parseInt(value - 1) }]
      }
      setTicks(freshTicks)
    }
  }
